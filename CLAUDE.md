...
1. `scripts/eval.py` — Core pipeline. Discovers YAML eval cases in `<skill-path>/evals/`, runs each sequentially (to avoid rate limits), grades responses, writes per-case outputs and `summary.json`. Retry logic with exponential backoff wraps both execute and grade calls via `_run_copilot()`. Calls use `--allow-all-tools`; each case uses an isolated per-case I/O workspace (seeded from case `files:`) that is shared by both execute and grade so graders can inspect generated artifacts without exposing `evals/`. `files:` entries can inline `content` or copy existing files/directories by `path`. Copilot is also granted default read/write access to existing `<skill-path>/scripts`, `<skill-path>/references`, and `<skill-path>/assets` directories; optional extra directories can be granted via `--add-dir` from the `allowed-paths` input.
...
YAML files in `<skill-path>/evals/` with fields: `name`, `prompt`, `files` (optional, each entry supports `path` and optional `content`), `criteria` (list), `expect_skill` (bool), `timeout`.
